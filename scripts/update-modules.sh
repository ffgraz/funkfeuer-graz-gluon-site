...
git -C site diff --quiet ./modules || {
	1>&2 echo "Your modules file is dirty, aborting."
	exit 1
}

LOCAL_BRANCH=$(git -C site branch --show-current)
[[ $LOCAL_BRANCH != *-updates ]] && LOCAL_BRANCH+=-updates

for MODULE in "PACKAGES_LUCI" "PACKAGES_CM"; do
	...
	[ "$(git branch --show-current)" != "${LOCAL_BRANCH}" ] && {
		git -C site switch -c "${LOCAL_BRANCH}" || git -C site switch "${LOCAL_BRANCH}"
	}
